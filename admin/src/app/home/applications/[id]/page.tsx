...
export default function ApplicationDetailsPage({
  ...
  useEffect(() => {
    if (applications) {
      setApplication(
        applications.find((application: any) => application?.id === id)
      );
    }
  }, [applications]);
      ...
      {application ? (
        <Tabs defaultValue="personal-informations" className="space-y-8">
          ...
          <TabsList className="flex justify-start space-x-8 h-[4rem] bg-slate-200 text-black">
            ...
            <TabsTrigger value="education" className="text-base h-full">
              Education
            </TabsTrigger>
            <TabsTrigger value="competition" className="text-base h-full">
              Competition
            </TabsTrigger>
          ...
          <TabsContent value="personal-informations">
            <div className="space-y-6">
              ...
              <Field label="Guardian full name">
                {renderText(application?.guardianFullName)}
              </Field>
              <Field label="Guardian phone number">
                {renderText(application?.guardianPhoneNumber)}
              </Field>
              <Field label="Relationship with guardian">
                {renderText(
              ...
              <Field label="Special conditions">
                {renderText(application?.specialConditions)}
              </Field>
            </div>
          </TabsContent>
          ...
          <TabsContent value="education">
            <div className="space-y-6">
              <Field label="Education levels">
                {renderText(educationLevelLabels[application?.educationLevel])}
              </Field>
              <Field label="Education program">
                {renderText(educationFieldLabels[application?.educationField])}
              </Field>
              <Field label="Highschool">
                {renderText(application?.highschool)}
              </Field>
              <Separator className="my-6" />

              <Field label="Average grade">
                {renderText(application?.averageGrade)}
              </Field>
              <Field label="Math average grade">
                {renderText(application?.mathAverageGrade)}
              </Field>
              <Field label="Ranking">{renderText(application?.ranking)}</Field>
              <Field label="Math ranking">
                {renderText(application?.mathRanking)}
              </Field>
              <Field label="Number of students in class">
                {renderText(application?.numberOfStudentsInClass)}
              </Field>
            </div>
          </TabsContent>
          ...
          <TabsContent value="competition">
            <div className="space-y-6">
              <Field label="Have you participated in competitions before (Olympiads, national contests...) ?">
                {renderText(
                  booleanLabels[application?.hasPreviouslyParticipated]
                )}
              </Field>
              <Field label="If yes, please specify which ones and the achieved result.">
                {renderText(application?.previousCompetitions)}
              </Field>
              <Field label="Have you participated in FMA 2023/2024 ?">
                {renderText(
                  booleanLabels[application?.hasPreviouslyParticipatedInMtym]
                )}
              </Field>
              <Separator className="my-6" />
              <Field label="Motivations">
                {renderText(application?.motivations)}
              </Field>
              <Field label="Comments">
                {renderText(application?.comments)}
              </Field>
            </div>
          </TabsContent>
